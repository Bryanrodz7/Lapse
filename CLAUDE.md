...
- **Do not put `&apos;` (or an escaped `'`) inside a `strings.xml` value on this toolchain.** aapt2 fails with *"Invalid unicode escape sequence in string"* and reports a line number ten lines off the real one, which sends you hunting in the wrong place. Use the typographic apostrophe `’` instead — it also looks better in a serif app.
- **Ads are gated behind UMP consent.** `ConsentManager` resolves consent first and only then initialises the Mobile Ads SDK; nothing may request an ad before that. `AnchoredAdBanner` takes a `ready` flag for the same reason, while still reserving its height so consent resolving never shifts the layout.
- **`enableEdgeToEdge()` with no arguments follows the *system* theme, not the app's.** The app has its own `ThemeMode`, so on `DARK` over a light system it left dark status bar icons on the dark background — measured at *zero* contrast, the clock and battery invisible rather than dim. It is also a one-shot call in `onCreate`, so it could never react to the theme being changed in Settings. `MainActivity` re-applies it from a `DisposableEffect(dark)` inside `setContent`, passing `SystemBarStyle.auto(...) { dark }`. Note the navigation bar scrims androidx uses by default are private, so they are duplicated in `MainActivity`; they matter below API 29, which minSdk 24 still covers. **Neither unit nor instrumented tests catch this** — it is window state, not composition. Check it by eye, or by sampling status bar pixels from a screenshot.
- **Glance day/night colour needs `glance-material3`.** Both `DayNightColorProvider` and the resource-based `ColorProvider(@ColorRes)` are `@RestrictTo` the Glance library — lint fails the build on them, and they can break on any Glance update. The public path is `GlanceTheme(colors = ColorProviders(light, dark))` from `androidx.glance:glance-material3`, which is why the widget's five status colours ride on Material scheme slots (see `LapseWidgetTheme`).
- **Glance's test harness provides no Android context**, so a widget composable that reads `LocalContext` throws *"No default context"*. Resolve strings and intents in `provideGlance` and pass a prepared model into the composable — `WidgetContent` exists for that reason.
- **Launcher widget placement cannot be automated from adb.** `input draganddrop` and discrete `motionevent` drags both fail against the Pixel launcher. Widget behaviour is verified by `NextExpiryWidgetTest` (rendered content, via `glance-appwidget-testing`) and `WidgetBindingTest` (a real bind on device, which needs `adb shell appwidget grantbind --package dev.randyapps.lapse.test`). Placing one by hand is still the only way to eyeball it on a home screen.
